...
export const DataProvider = ({ children }) => {
  ...
  const autoGenerateWeekSchedule = async (weekId, weekStart) => {
    // Recurring entries are stored with weekId:'recurring' and shown automatically
    // in every week via the screen-level filter. Week-to-week copying is no longer needed.
    return { success: true, message: 'Recurring entries appear automatically', count: 0 };
  };
